...
type Certificate =
  RouterOutput["certificates"]["getUserCertificates"]["items"][number];
...
export function CertificateSummary() {
  ...
  const [selectedSort, setSelectedSort] =
    useState<string>("creationDateNewest");

  let sortBy: "name" | "email" | "date" = "date";
  ...
  if (selectedSort === "emailAToZ") {
    sortBy = "email";
    sortDir = "asc";
  }
  if (selectedSort === "emailZToA") {
    sortBy = "email";
    sortDir = "desc";
  }
  ...
  const {
    ...
    fetchNextPage,
    isFetching,
  } = api.certificates.getUserCertificates.useInfiniteQuery(
    {
      limit: 20,
  ...
  const userCertificates = data?.pages.flatMap((page) => page.items) ?? [];
  ...
  const filteredCertificates = (userCertificates ?? [])
    .filter((certificate) => {
      ...
      if (searchValue.length > 0 && createdDate.includes(searchValue)) {
        return true;
      }
      return true;
    })
  ...
  return (
    <div className="flex h-[75vh] min-h-full flex-col items-center rounded-lg border border-dashed pt-5 text-center">
      ...
      ) : userCertificates?.length === 0 && !searchValue ? (
        ...
        <div className="flex w-full flex-1 flex-col overflow-hidden px-5 pb-5">
          <div className="flex-1 overflow-y-auto rounded-md border">
            ...
            <div className="bg-background text-muted-foreground sticky top-0 z-10 hidden items-center gap-4 border-b px-5 py-3 text-sm md:grid md:grid-cols-[70px_1fr_100px_80px_40px] lg:grid-cols-[70px_1fr_1.5fr_100px_80px_100px_40px]">
              ...
              <span className="hidden text-left lg:block">ID certifikátu</span>
              <div /> {/* Místo pro akce */}
            </div>
            ...
            <div className="flex flex-col">
              {filteredCertificates.map((certificate) => (
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 overflow-hidden">
                    <Link href={`/dashboard/me-certifikaty/${certificate.id}`}>
                          ...
                          onClick={async () => {
                            try {
                              toast.loading("Připravuji stahování...", {
                                id: `download-${certificate.id}`,
                              });
                              const data =
                                await utils.certificates.getCertificateUrl.fetch(
                                  { id: certificate.id },
                                );
                              if (data.certificateUrl) {
                                ...
                                toast.error("Certifikát nemá platný obrázek", {
                                  id: `download-${certificate.id}`,
                                });
                              }
                            } catch {
              ...
              {filteredCertificates.length === 0 && searchValue && (
                <div className="text-muted-foreground p-8 text-center">
                  Žádný certifikát neodpovídá vašemu vyhledávání.
                </div>
              )}
              ...
              {(hasNextPage || isFetchingNextPage) && (
                ...
                >
                  ...
                  <span>Načítám další certifikáty...</span>
                </div>
              )}
            </div>
          </div>
